...
class Main extends Controller
{
  ...
  function generateQR(){
    ...
    return 'DONE';
        ...
        //return $img->move(time().'.png',public_path('generated-qr'));
        // return response($img)->header('Content-type','image/png');

    // foreach ($trees as  $t) {

    //   $path = public_path('generated-qr/'.time().'.png');
    //   $string = $t->name.','.$t->species.','.$t->tree_number.','.$t->id;
    //   echo $string;
    //   $code  = new BaconQrCodeGenerator;
    //   $code -> backgroundColor(58, 22, 106)->color(255, 0, 127)
    //     ->format('png')->merge('default-imgs/100k-footer.png', 0.3, true)->size(300)
