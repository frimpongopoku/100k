...
import FoodSelection from './Selection'; 
...
export default class Cooks extends Component {
  constructor (props){
    ...
    this.state ={ allPastries:[],items:[], quantity:[],prices:[],destination:null, allCenters:[]};
  }
  ...
  shipValues(){
    $.ajax({
      method:'get',
      url:'/receive-values-from/kitchen',
      data:{
        description:this.prepareForDatabase(),
        destination:this.state.destination}
    })
    .done(function(){
      window.location = "/cooks/home";
    })
  }
  ...
  componentDidMount() {
    ...
    render() {
        ...
        <div>
          ...
          <ItemView  allCenters = {this.state.allCenters} destination={this.setDestination} deleteFxn ={this.delItem} items={this.state.items} q ={this.state.quantity} prices ={this.state.prices}/>
            ...
            <div className="modal-dialog modal-md">
              <div className="modal-content">
                ...
                <div className="modal-body" style={{maxHeight:'250px',overflowY:'scroll'}}>
                  <p>Final Values To Be Sent To <span class="text text-success" style={{fontWeight:700}}>{this.state.destination}</span> </p>
...
if (document.getElementById('cooks-react-div')) {
    ReactDOM.render(<Cooks />, document.getElementById('cooks-react-div'));
}
